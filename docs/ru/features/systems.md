...
public interface ISystem {
    ...
    // Хуки сериализации в снапшот — переопределите Write() и Read(), чтобы сохранять состояние системы
    Guid? Guid()                                              => null;
    byte  Version()                                            => 0;
...
`ISystem` имеет четыре опциональных метода с дефолтной реализацией (`Guid?`, `Version`, `Write`, `Read`) — та же форма, что и у `IResource`. Переопределите и `Write`, и `Read`, чтобы подключить собственное состояние инстанса системы к сериализации:
...
У каждой системы ровно один идентификатор: `Guid()`, возвращающий `null` (по умолчанию), выводит его из имени типа системы, а переопределение `Guid()` фиксирует его явно. По этому идентификатору записи снапшота сопоставляются с зарегистрированными системами — и сериализованное состояние системы, и её тик трекинга (тик мира, на котором она отработала в прошлый раз; он ограничивает окно изменений для `AllAdded<T>` / `AllChanged<T>` / `AllDeleted<T>` внутри неё). Тик сохраняется для каждой системы, независимо от наличия у неё сериализуемого состояния.

Валидация выполняется при `Add<TSystem>`:

- Переопределение и `Write`, и `Read` включает сериализацию собственного состояния системы; если не переопределён ни один из них, система в этой части снапшота не участвует. Переопределение ровно одного из двух выбрасывает `StaticEcsException`.
- Дубликат идентификатора внутри одной группы `Systems<TSystemsType>` ассертится в DEBUG — несколько инстансов одного типа системы регистрируются с переопределённым `Guid()` и разными значениями.

Каждый `Systems<TSystemsType>.Create` регистрирует свою группу в реестре снапшотов мира; `Guid` группы по умолчанию = `typeof(TSystemsType).GuidFromAQN()` и переопределяется опциональным параметром `snapshotGuid`. `WorldSnapshot` автоматически пишет одну секцию на группу (её scoped-ресурсы + по записи на каждую систему); при загрузке секции с незарегистрированным `Guid` группы молча пропускаются.
