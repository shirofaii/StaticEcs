...
`ISystem` имеет те же четыре опциональных метода, что и `IResource`. Переопределение сразу и `Write`, и `Read` подключает собственное состояние системы к сериализации.
...
У каждой системы ровно один идентификатор. `Guid()`, возвращающий `null` (по умолчанию), означает, что идентификатор выводится автоматически из имени типа системы (`assembly-qualified name`); переопределите `Guid()`, чтобы зафиксировать его явно. Записи снапшота — и тик трекинга системы, и её сериализованное состояние — сопоставляются с зарегистрированными системами по этому идентификатору.

#### Правила валидации (проверяются при `Add<TSystem>`)

- Переопределение и `Write`, и `Read` включает сериализацию собственного состояния системы; если не переопределён ни один из них, система в этой части снапшота не участвует. Переопределение ровно одного из двух выбрасывает `StaticEcsException` из `Add`.
- Дубликат идентификатора внутри одной группы `Systems<TSystemsType>` ассертится в DEBUG. Поэтому регистрация нескольких инстансов одного типа системы требует переопределить `Guid()` разными значениями для каждого инстанса.
...
Каждая секция группы содержит её scoped-ресурсы (singleton + named) и по записи на каждую систему: идентификатор, тик трекинга и — для систем, переопределяющих `Write`/`Read`, — их сериализованное состояние.

#### Тики трекинга по системам

У каждой системы, переопределяющей `Update()`, есть собственный тик трекинга — тик мира, на котором она отработала в прошлый раз. Он служит нижней границей окна изменений, которое видят `AllAdded<T>` / `AllChanged<T>` / `AllDeleted<T>` внутри этой системы. Тик сохраняется для **каждой** системы, независимо от того, переопределяет ли она `Write`/`Read`, поэтому системы без сериализуемого состояния тоже его восстанавливают. При загрузке восстановленный тик зажимается в диапазон, ещё покрытый кольцевым буфером трекинга (`[CurrentTick - TrackingBufferSize, CurrentTick]`).

Тики восстанавливаются при обоих порядках загрузки:

```csharp
// Группа уже инициализирована — тики применяются к работающему списку обновления
W.Serializer.LoadWorldSnapshot(snapshot);

// Пересобранная группа — Initialize берёт для каждой системы загруженный тик
...
Система, которой нет в снапшоте — добавлена после сохранения, сменился её идентификатор или её группа создана позже — получает тик мира, восстановленный из снапшота. Поэтому её первое обновление после загрузки видит пустое окно изменений и не сообщает ни об одном отслеженном изменении.
